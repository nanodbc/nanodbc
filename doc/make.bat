...
if "%1" == "clean" (
	for /d %%i in (%DOXYGENDIR%\xml\*) do rmdir /q /s %%i
	rmdir /q /s %DOXYGENDIR%\xml
	for /d %%i in (%DOXYGENDIR%\html\*) do rmdir /q /s %%i
	rmdir /q /s %DOXYGENDIR%\html
...
REM set SPHINXBUILD=python -m sphinx.__init__
...
if "%1" == "doxygen" (
	pushd ..
	doxygen docs/%DOXYGENDIR%/doxygen.conf
