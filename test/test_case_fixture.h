...
struct test_case_fixture : public base_test_fixture
{
    ...
    void test_catalog_tables()
    {
        ...
        // Create the table before looking for anything, so that the counts below do not
        // depend on an earlier test having left one behind. Against a database that starts
        // out empty they find nothing and the test fails, which is what makes this one
...
void test_catalog_table_privileges()
{
    ...
    create_table(connection, NANODBC_TEXT("test_catalog_table_privileges"), NANODBC_TEXT("i int"));
...
void test_column_descriptor()
{
    ...
    if (vendor_ == database_vendor::sqlite)
    {
        ...
        REQUIRE(
            result.column_datatype(1) == -9); // FIXME: If not SQL_WVARCHAR(-9), what is this type?
    ...
    {
        REQUIRE(
            (result.column_datatype(1) == SQL_DECIMAL || result.column_datatype(1) == SQL_NUMERIC));
        REQUIRE(result.column_c_datatype(1) == SQL_C_CHAR);
        REQUIRE(!result.column_unsigned(1));
    }
    REQUIRE(result.column_size(1) == 7);
    ...
    {
        REQUIRE(
            (result.column_datatype(2) == SQL_DECIMAL || result.column_datatype(2) == SQL_NUMERIC));
...
void test_decimal_conversion()
{
    ...
    execute(connection, NANODBC_TEXT("create table test_decimal_conversion (d decimal(9, 3));"));
...
void test_datasources()
{
    ...
    if (test_dsn_found)
    {
        ...
        bool found = std::any_of(
            ...
            { return dsn.name == nanodbc::test::convert("testdsn") && dsn.driver == driver_name; });
        REQUIRE(found);
    }
}
...
void test_error()
{
    ...
    {
        create_table_sql = create_table_sql + NANODBC_TEXT(" constraint test_error_pk primary key");
    }
    if (vendor_ == database_vendor::vertica)
    {
...
void test_exception()
{
    ...
    results = execute(connection, NANODBC_TEXT("select * from test_exception where i is null;"));
...
PRIMARY KEY(t2_fid)
    ...
    // table
    {
        nanodbc::string sql = NANODBC_TEXT("SELECT t1_fid1 AS fid1, t1_fid2, name FROM t1 AS t");
        if (vendor_ == database_vendor::sqlserver)
...
void test_integral_small_types_batch()
{
    ...
    auto const sql =
        NANODBC_TEXT("insert into test_integral_small_types_batch (sc, uc, b) values (?, ?, ?);");
...
void test_null_with_bound_columns_unbound()
{
    ...
    nanodbc::result results = execute(
        conn, NANODBC_TEXT("select a, b from test_null_with_bound_columns_unbound order by a;"));
    results.unbind();
...
void test_result_iterator()
{
    ...
    execute(connection, NANODBC_TEXT("create table test_result_iterator (i int, s varchar(10));"));
...
void test_statement_usable_when_result_gone()
{
    ...
    execute(
        c, NANODBC_TEXT("insert into test_statement_usable_when_result_gone values (1, 'one');"));
    execute(
        c, NANODBC_TEXT("insert into test_statement_usable_when_result_gone values (2, 'two');"));
    execute(
        c, NANODBC_TEXT("insert into test_statement_usable_when_result_gone values (3, 'tri');"));
...
void test_simple()
{
    ...
    {
        nanodbc::result results =
            execute(connection, NANODBC_TEXT("select a, b from test_simple order by sort_order;"));
...
void test_string_with_varchar_max()
{
    ...
    execute(
        connection, NANODBC_TEXT("create table test_string_with_varchar_max (s varchar(max));"));
    execute(
        connection,
...
void test_string_vector()
{
    ...
    nanodbc::statement query(connection);
    prepare(
        query, NANODBC_TEXT("insert into test_string_vector(first, last, gender) values(?, ?, ?)"));
...
// std::string_view is only supported since C++17 compliant compilers
    ...
    const std::vector<nanodbc::string_view> last_name(last_name_str.begin(), last_name_str.end());
    ...
    prepare(
        query,
        NANODBC_TEXT("insert into test_string_view_vector(first, last, gender) values(?, ?, ?)"));
    REQUIRE(query.parameters() == 3);
    ...
    nanodbc::result results =
        execute(connection, NANODBC_TEXT("select first,last,gender from test_string_view_vector"));
    REQUIRE(results.next());
    REQUIRE(results.get<nanodbc::string>(0) == NANODBC_TEXT("Fred"));
    REQUIRE(results.get<nanodbc::string>(1) == NANODBC_TEXT("Flintstone"));
    REQUIRE(results.get<nanodbc::string>(2) == NANODBC_TEXT("Male"));
...
void test_transaction()
{
    ...
    if (vendor_ == database_vendor::mysql)
        execute(connection, NANODBC_TEXT("create table test_transaction (i int) ENGINE = INNODB;"));
    else
...
void test_win32_variant()
{
    ...
    // prepare test table
    {
        ...
        prepare(
            st,
            NANODBC_TEXT("insert into test_win32_variant(i,f,d,dt,t,s,b) values (?,?,?,?,?,?,?);"));
...
void test_win32_variant_null()
{
    ...
    execute(
        ...
        NANODBC_TEXT("create table ") + table_name + NANODBC_TEXT("(") +
            ...
            NANODBC_TEXT("c7 ") + text_type_name + NANODBC_TEXT(" NULL,") + NANODBC_TEXT("c8 ") +
            binary_type_name + NANODBC_TEXT(" NULL);"));
...
void test_win32_variant_row_cached_result()
{
    ...
    create_table(
        ...
        NANODBC_TEXT("(i0 int, s1 varchar(60), f2 float, s3 varchar(60), d4 decimal(9, 3), b5 ") +
    ...
    for (short v : {0, 1, 2})
    {
        ...
        std::vector<std::vector<std::uint8_t>> b5 = {from_hex("11f830737ff4bc41a4ffe792d073f41f")};
...
void test_while_not_end_iteration()
{
    ...
    nanodbc::result results = execute(
        connection, NANODBC_TEXT("select * from test_while_not_end_iteration order by 1 desc;"));
    int i = 3;
...
void test_batch_insert_describe_param()
{
    ...
    nanodbc::string insert(
        NANODBC_TEXT("insert into test_batch_insert_param_type (i, s, f, d) values(?, ?, ?, ?)"));
    nanodbc::statement stmt(conn);
    prepare(stmt, insert);
    ...
    nanodbc::transact(stmt, batch_size);
    {
        ...
        {
            REQUIRE(result.get<int>(0) == integers[i]);
            REQUIRE(result.get<float>(1) == floats[i]); // exact test might fail, switch to Approx
            REQUIRE(result.get<nanodbc::string>(2) == strings[i]);
