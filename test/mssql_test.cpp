...
TEST_CASE_METHOD(mssql_fixture, "test_string_with_interrupting_zeros", "[mssql][string]")
{
    nanodbc::connection connection = connect();
    drop_table(connection, NANODBC_TEXT("test_string_with_interrupting_zeros"));
    execute(
        connection,
        NANODBC_TEXT("create table test_string_with_interrupting_zeros (s nvarchar(max));"));

    // 1022 A
    nanodbc::string chunk1 = NANODBC_TEXT(
    ...
    // 1020 B
    nanodbc::string chunk2 = NANODBC_TEXT(
    ...
    nanodbc::string select;
    results.get_ref(0, select);
    REQUIRE(select == chunk1 + chunk2 + chunk3);
}
